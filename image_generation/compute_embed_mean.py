...
def compute_embeds(batch_size=16, num_gpus=1):
    training_set_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=TRAIN_DATA_PATH, use_labels=False, max_size=None, xflip=False, use_clip=True, ratio=1., remove_mean=False, add_noise=False, noise_level=0., add_lafite_noise=False)
        
    data_loader_kwargs = dnnlib.EasyDict(pin_memory=False, num_workers=0) #, prefetch_factor=2)
    
    training_set = dnnlib.util.construct_class_by_name(**training_set_kwargs)
    
    training_set_loader = torch.utils.data.DataLoader(dataset=training_set, batch_size=batch_size//num_gpus, **data_loader_kwargs)

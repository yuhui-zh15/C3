...
class Loss:
    def accumulate_gradients(
        self, phase, real_img, real_c, gen_z, gen_c, sync, gain, real_features
...
class Model(torch.nn.Module):
    ...
    def loss(self, real, fake, temp=0.1, lam=0.5):
        ...
        if temp > 0.0:
            ...
            if 0.0 < lam < 1.0:
                return -(lam * torch.log(sim1) + (1.0 - lam) * torch.log(sim2))
            elif lam == 0:
...
class StyleGAN2Loss(Loss):
    def __init__(
        ...
        self.mapper.load_state_dict(
            torch.load("./implicit.0.001.64.True.0.0.pth", map_location="cpu")
        )  # path to the noise mapping network
        ...
        if remove_mean:
            with open(IMAGE_EMBED_MEAN_LAFITE, "rb") as f:
                self.image_embed_mean = pickle.load(f).to(device)
        ...
        add_noise_bool = self.add_noise or self.add_lafite_noise
        norm_after_remove_mean_nool = "N/A"
        if self.remove_mean:
            if not (self.add_noise or self.add_lafite_noise):
                norm_after_remove_mean_nool = True
            else:
                norm_after_remove_mean_nool = norm_after_remove_mean

        print("=" * 80)
        print("StyleGAN2Loss params:")
        print(f"Remove mean: {remove_mean}")
        print(f"Add noise: {add_noise}")
        print(f"Add Lafite noise: {add_lafite_noise}")
        print(f'Noise level: {noise_level if add_noise_bool else "N/A"}')
        print(f"Normalize after removing mean: {norm_after_remove_mean}")
        print("=" * 80)
    ...
    def full_preprocess(self, img, mode="bicubic", ratio=0.5):
        ...
        reshaped_img = (reshaped_img + 1.0) * 0.5  # range in [0., 1.] now
        reshaped_img = self.normalize()(reshaped_img)
        return reshaped_img

    def custom_preprocess(
        self, img, ind, cut_num, mode="bicubic"
        ...
        cut_size = torch.randint(
            int(full_size // (grid + 1)),
            int(min(min(full_size // 2, most_right), most_bottom)),
            (),
    ...
    def accumulate_gradients(
        ...
        if txt_fts.shape[-1] == img_fts.shape[-1]:
            # Remove mean
            img_fts_ = img_fts / img_fts.norm(dim=-1, keepdim=True)
            if self.remove_mean:
                img_fts_ = img_fts_ - self.image_embed_mean

                if self.norm_after_remove_mean:
                    img_fts_ = img_fts_ / img_fts_.norm(dim=-1, keepdim=True)

            # Gaussian perturbation (note: weighted average of normed noise)
            if self.add_lafite_noise:
                img_fts_ = img_fts_ / img_fts_.norm(dim=-1, keepdim=True)
                random_noise = random_noise / random_noise.norm(dim=-1, keepdim=True)
                img_fts_ = img_fts_ * (1 - aug_level_2) + random_noise * aug_level_2
            elif self.add_noise:
                img_fts_ = img_fts_ + random_noise * aug_level_2
            ...
            if mixing_prob > 0.99:
                ...
                txt_fts_ = torch.where(
                    torch.rand([txt_fts_.shape[0], 1], device=txt_fts_.device)
                    < mixing_prob,
                    img_fts_,
                    txt_fts_,
                )
        ...
        def gather_tensor(input_tensor, gather_or_not):
            if gather_or_not:
                ...
                output_tensor = [
                    torch.zeros_like(input_tensor) for _ in range(world_size)
                ]
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function("Gmain_forward"):
                gen_img, _gen_ws = self.run_G(
                    gen_z, gen_c, txt_fts=txt_fts_, sync=(sync and not do_Gpl)
                )  # May get synced by Gpl.
                gen_logits, gen_d_fts = self.run_D(
                    gen_img, gen_c, sync=False, fts=txt_fts_
                )
                ...
                img_fts_gen_full = img_fts_gen_full / img_fts_gen_full.norm(
                    dim=-1, keepdim=True
                )
                ...
                if img_txt_c > 0.0:
                    clip_loss_img_txt = self.contra_loss(
                        temp, img_fts_gen_full_all, txt_fts_all, lam
                    )
                    loss_Gmain = loss_Gmain - img_txt_c * clip_loss_img_txt.mean()

                if img_img_c > 0.0:
                    clip_loss_img_img = self.contra_loss(
                        temp, img_fts_gen_full_all, img_fts_all, lam
                    )
                ...
                if img_txt_d > 0.0:
                    loss_Gmain = (
                        loss_Gmain
                        - img_txt_d
                        * self.contra_loss(temp, gen_d_fts_all, txt_fts_all, lam).mean()
                    )
                if img_img_d > 0.0:
                    with torch.no_grad():
                        _, g_real_d_fts = self.run_D(
                            real_img.detach(), real_c, sync=False, fts=txt_fts_
                        )
                    ...
                    loss_Gmain = (
                        ...
                        - img_img_d
                        * self.contra_loss(
                            temp, g_real_d_fts_all, gen_d_fts_all, lam
                        ).mean()
                    )
        ...
        if do_Gpl:
            with torch.autograd.profiler.record_function("Gpl_forward"):
                ...
                gen_img, gen_ws = self.run_G(
                    gen_z[:batch_size], gen_c[:batch_size], txt_fts=txt_fts_0, sync=sync
                )
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function("Dgen_forward"):
                gen_img, _gen_ws = self.run_G(
                    gen_z, gen_c, txt_fts=txt_fts_, sync=False
                )
                gen_logits, gen_d_fts = self.run_D(
                    gen_img, gen_c, sync=False, fts=txt_fts_
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + "_forward"):
                ...
                real_logits, real_d_fts = self.run_D(
                    real_img_tmp, real_c, sync=sync, fts=txt_fts_
                )
                ...
                if do_Dmain:
                    ...
                    if img_txt_d > 0.0:
                        ...
                        loss_Dreal = (
                            ...
                            - img_txt_d
                            * self.contra_loss(
                                temp, real_d_fts_all, txt_fts_all, lam
                            ).mean()
                        )

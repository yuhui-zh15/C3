...
class Conv2dLayer(torch.nn.Module):
    ...
    def forward(self, x, gain=1, emb=None):
...
@persistence.persistent_class
class ManiNetwork(torch.nn.Module):
    def __init__(
        self,
        z_dim,  # Input latent (Z) dimensionality, 0 = no latent.
        c_dim,  # Conditioning label (C) dimensionality, 0 = no label.
    ...
    def forward(
        self, z, c, w, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False
    ):
        # Embed, normalize, and concat inputs.
        x = None
        with torch.autograd.profiler.record_function("input"):
...
class SynthesisLayer(torch.nn.Module):
    def __init__(
        ...
        if self.structure == 0:
            self.affine = FullyConnectedLayer(
                w_dim, in_channels, bias_init=1
            )  # initial affine style layer

            self.pre_0 = FullyConnectedLayer(
                f_dim, _f_dim, activation="lrelu", lr_multiplier=0.01
            )  # pre process text features
            self.pre_1 = FullyConnectedLayer(
                _f_dim, 512, activation="lrelu", lr_multiplier=0.01
            )

            self.affine_0 = FullyConnectedLayer(
                512, in_channels, bias_init=1
            )  # f([w, txt])

        elif self.structure == 1:
            self.affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)  # wed
            self.scale_layer_0 = FullyConnectedLayer(
                f_dim, _f_dim, activation="lrelu", lr_multiplier=0.01
            )  # wed
            self.scale_layer_1 = FullyConnectedLayer(
                _f_dim, 512, activation="lrelu", lr_multiplier=0.01
            )  # wed
            self.scale_layer_2 = FullyConnectedLayer(
                512,
                in_channels,
            )  # , lr_multiplier=0.01)#wed

            self.bias_layer_0 = FullyConnectedLayer(
                f_dim, _f_dim, activation="lrelu", lr_multiplier=0.01
            )  # wed
            self.bias_layer_1 = FullyConnectedLayer(
                _f_dim, 512, activation="lrelu", lr_multiplier=0.01
            )  # wed
            self.bias_layer_2 = FullyConnectedLayer(
                512,
                in_channels,
            )  # , lr_multiplier=0.01)#wed
            ...
            self.affine_0 = FullyConnectedLayer(
                w_dim + 512, in_channels, bias_init=1
            )  # f([w, txt])
    ...
    def forward(
        self, x, w, fts, styles=None, noise_mode="random", fused_modconv=True, gain=1
    ):
...
class ToRGBLayer(torch.nn.Module):
    ...
    def forward(self, x, w, fts, styles=None, fused_modconv=True):
        if styles is None:
            if self.structure == 0:
                fts = self.pre_0(fts)
                fts = self.pre_1(fts)
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(
        ...
        **layer_kwargs,
        ...
        # Main layers.
        if self.in_channels == 0:
            x, style_init, style_res = self.conv1(
                x,
                next(w_iter),
                styles=next(s_iter),
                fts=fts,
            ...
            x, style_init, style_res = self.conv0(
                x,
                next(w_iter),
                styles=next(s_iter),
                fts=fts,
        ...
        if self.is_last or self.architecture == "skip":
            y, style_init, style_res = self.torgb(
                x,
                next(w_iter),
                styles=next(s_iter),
                fts=fts,
        ...
        **block_kwargs,
        ...
        with torch.autograd.profiler.record_function("split_ws"):
            ...
            for res in self.block_resolutions:
                ...
                if styles is not None:
                    block_styles.append(
                        styles[s_idx : s_idx + block.num_conv + block.num_torgb]
                    )
                else:
        ...
        styles_init = []
        styles_res = []
        for res, cur_ws, cur_style in zip(
            self.block_resolutions, block_ws, block_styles
        ):
            block = getattr(self, f"b{res}")
            x, img, style_list, style_init_list, style_res_list = block(
                x, img, cur_ws, fts=fts, styles=cur_style, **block_kwargs
            )

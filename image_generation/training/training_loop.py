...
def training_loop(
    ...
    image_snapshot_ticks=150,  # How often to save image snapshots? None = disable.
    network_snapshot_ticks=150,  # How often to save network snapshots? None = disable.
    ...
    if rank == 0:
        ...
        with torch.no_grad():
            ...
            for step in range(len(text)):
                ...
                txt_fts = (
                    clip_model.encode_text(tokenized_text)
                    .view(1, -1)
                ...
                for f, z, c, w in zip(f_txt_, grid_z, grid_c, previous_w_list):
                    img_, w_ = G_ema(
                        z=z, c=c, fts=f, return_w=True, w=w, noise_mode="const"
                    )
                ...
                save_image_grid(
                    images,
                    os.path.join(run_dir, "fakes_init_txt_step_%i.png" % step),
                    drange=[-1, 1],
                    grid_size=grid_size,
                )
            del clip_model
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function("data_fetch"):
            phase_real_img, phase_real_c, phase_img_features, phase_txt_features = next(
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for round_idx, (
                ...
                loss.accumulate_gradients(
                    ...
                    sync=sync,
                    gain=gain,
                    img_fts=real_img_feature,
                    txt_fts=real_txt_feature,
        ...
        if (
            ...
            for step in range(len(text)):
                ...
                save_image_grid(
                    images,
                    os.path.join(
                        run_dir, f"fakes_{cur_nimg//1000:06d}txt_step_{step}.png"
                    ),
                    drange=[-1, 1],
                    grid_size=grid_size,
                )
                img_list = []
                for f, z, c, s, w in zip(
                    txt_fts_list[step], grid_z, grid_c, style_list, w_list
                ):
                    w = w.normal_()
                    #                     img_ = G_ema.synthesis(ws=w, fts=f, styles=s, ) # test whether we can construct using style
                    img_ = G_ema.synthesis(
                        ws=w, fts=f, styles=s, noise_mode="const"
                    )  # test whether we can construct using style
                    img_list.append(img_)
                images = torch.cat([img.cpu() for img in img_list]).numpy()
                save_image_grid(
                    images,
                    os.path.join(
                        run_dir, f"fakes_{cur_nimg//1000:06d}style_txt_step_{step}.png"
                    ),
                    drange=[-1, 1],
                    grid_size=grid_size,
                )
        ...
        if (network_snapshot_ticks is not None) and (
            ...
            snapshot_data[
                "args"
            ] = dict(  # newly added, not available in our pre-trained models
                num_gpus=num_gpus,
        ...
        if (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main.calc_metric(
                    ...
                    testset_kwargs=testing_set_kwargs,
                    num_gpus=num_gpus,
                    rank=rank,
                    device=device,
                    txt_recon=True,
                    img_recon=False,
                    metric_only_test=metric_only_test,
                )
                if rank == 0:
                ...
                wandb.log(result_metrics_log, step=global_step)

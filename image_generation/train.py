...
def get_experiment_name(args):
    ...
    if args.remove_mean and (args.add_noise or args.add_lafite_noise) and args.norm_after_remove_mean:
...
def setup_training_loop_kwargs(
    ...
    if test_data is None:
        test_data = data
    args.training_set_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data, use_labels=True, max_size=None, xflip=False, use_clip=True, ratio=args.ratio, remove_mean=remove_mean, add_noise=add_noise, noise_level=noise_level, add_lafite_noise=add_lafite_noise, norm_after_remove_mean=norm_after_remove_mean)
    args.testing_set_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=test_data, use_labels=True, max_size=None, xflip=False, use_clip=True, ratio=1.0, remove_mean=remove_mean)
...
#     args.M_kwargs = dnnlib.EasyDict(class_name='training.networks.ManiNetwork', z_dim=args.f_dim,  layer_features=args.f_dim, w_dim=512, num_layers=8)
    ...
    args.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.StyleGAN2Loss', r1_gamma=spec.gamma, remove_mean=remove_mean, add_noise=add_noise, noise_level=noise_level, add_lafite_noise=add_lafite_noise, norm_after_remove_mean=norm_after_remove_mean)

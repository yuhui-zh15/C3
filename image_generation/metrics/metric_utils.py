...
def compute_feature_stats_for_dataset(
    ...
    **stats_kwargs,
    ...
    if opts.cache:
        ...
        if opts.metric_only_test:
            args = dict(
                dataset_kwargs=opts.testset_kwargs,
    ...
    for images, _labels, img_fts, txt_fts in torch.utils.data.DataLoader(
        dataset=dataset,
        sampler=item_subset,
    ...
    **stats_kwargs,
    ...
    if opts.img_recon:
        ...
        if "celeba" in opts.testset_kwargs["path"].lower():
            ...
            num_items = (
                30000  # 30000 images are used for MS-COCO, CUB and LN-COCO datasets
            )
        stats = FeatureStats(max_items=num_items, **stats_kwargs)
        progress = opts.progress.sub(
            tag="generator features", num_items=num_items, rel_lo=rel_lo, rel_hi=rel_hi
        )
        detector = get_feature_detector(
            url=detector_url,
            device=opts.device,
        ...
        item_subset = [
            (i * opts.num_gpus + opts.rank) % len(dataset)
            for i in range((len(dataset) - 1) // opts.num_gpus + 1)
        ]
        ...
        while not stats.is_full():
            ...
            with torch.no_grad():
                clip_img_features = img_fts / img_fts.norm(
                    dim=-1, keepdim=True
                )  # .view((batch_size, -1))
                z = torch.randn([img_fts.size()[0], G.z_dim], device=opts.device)
                imgs = G(
                    z=z,
                    c=_labels.to(opts.device),
                    fts=clip_img_features.to(opts.device),
                )
        ...
        while not stats.is_full():
            ...
            with torch.no_grad():
                clip_txt_features = txt_fts / txt_fts.norm(
                    dim=-1, keepdim=True
                )  # .view((batch_size, -1))
                ...
                imgs = G(
                    z=z,
                    c=_labels.to(opts.device),
                    fts=clip_txt_features.to(opts.device),
                )
                imgs = (imgs * 127.5 + 128).clamp(0, 255).to(torch.uint8)

...
from . import datasets, lightning, losses
    ...
    if cfg.train.scheduler.name is not None:
        ...
        if scheduler_name == "linear_schedule_with_warmup":
            scheduler = get_linear_schedule_with_warmup(
                optimizer,
                num_warmup_steps=cfg.train.scheduler.warmup_steps,
                num_training_steps=cfg.lightning.trainer.max_epochs
                * cfg.data.num_batches,
            )
    return scheduler

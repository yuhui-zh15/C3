...
class InputInstance(object):
    ...
    def __init__(
        self, guid, text_a, text_b=None, label=None, score=None, img_key=None, q_id=None
    ):
...
class NLVRProcessor(DataProcessor):
    ...
    def get_train_examples(
        self, data_dir, use_label_seq=True, file_name="nlvr2_train.json"
    ):
        """See base class."""
    ...
    def get_dev_examples(
        self, data_dir, use_label_seq=True, file_name="nlvr2_dev.json"
    ):
        """See base class."""
    ...
    def get_test_examples(
        self, data_dir, use_label_seq=True, file_name="nlvr2_test1.json"
    ):
        """See base class."""
    ...
    def _create_examples(self, lines, set_type, use_label_seq=True):
        ...
        for i, line in enumerate(lines):
            ...
            label = line[
                "label"
            ]  # None if set_type.startswith('test') else line['label']
            score = 0
...
class VCR_Q_A_Processor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for i, line in enumerate(lines):
            ...
            img_key = line["img_id"]
            q_id = int(
                line["annot_id"].split("-")[-1]
            )  # int(line['q_id']) if set_type.startswith('test') else 0
...
class VCR_QA_R_Processor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for i, line in enumerate(lines):
            ...
            label = (
                None if set_type.startswith("test") else line["rational_label"]
            )  # rational_label
            img_key = line["img_id"]
            q_id = int(
                line["annot_id"].split("-")[-1]
    ...
    def _create_examples(self, lines, set_type):
        ...
        for i, line in enumerate(lines):
            ...
            if set_type == "train":  # qa -> r
                ...
                text_a = (
                    line["q"] + " " + line["choices"][line["label"]]
                )  # question_choice
                choices = line["rational_choices"]  # rational_choice
                label = (
                    None if set_type.startswith("test") else line["rational_label"]
...
def convert_examples_to_features_vqa(
    examples,
    img_feats,
    label_list,
    max_img_seq_length,
    ...
    for ex_index, example in enumerate(examples[0:]):
        ...
        if img_feat.shape[0] > max_img_seq_length:
            ...
            if max_img_seq_length > 0:
                input_mask = (
                    input_mask
                    + [1 if mask_padding_with_zero else 0] * img_feat.shape[0]
                )
            ...
            padding_matrix = torch.zeros(
                (max_img_seq_length - img_feat.shape[0], img_feat.shape[1])
            )
            img_feat = torch.cat((img_feat, padding_matrix), 0)
            if max_img_seq_length > 0:
                input_mask = input_mask + (
                    [0 if mask_padding_with_zero else 1] * padding_matrix.shape[0]
                )
